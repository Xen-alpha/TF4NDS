...
int getLoadedTextureCount() {
    return numTexturesLoaded;
}

extern int textureIDs[MAX_TEX_DATA];
...
void drawTexturedFace(const dmap_t *map, const dface_t* face) {
    ...
    if (textureID < 0) {
        textureID = loadTexture(numTexturesLoaded, texture->name, texture->width, texture->height);
        if (textureID < 0) {
            printf("Failed to load texture: %s\n", texture->name);
            return;
        }
        if (numTexturesLoaded >= MAX_TEX_DATA) {
            printf("Warning: Maximum texture limit reached. Rendering will not work as expected.\n");
        } else {
          numTexturesLoaded++;
        }
    }
